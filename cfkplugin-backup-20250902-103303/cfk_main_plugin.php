...
class ChristmasForKidsPlugin {
    ...
    private function create_database_tables(): void {
        ...
        $result1 = dbDelta($sql_sponsorships);
        error_log("CFK sponsorships table: " . print_r($result1, true));
        
        // Create email log table  
        $result2 = dbDelta($sql_email_log);
        error_log("CFK email_log table: " . print_r($result2, true));
        ...
        if ($wpdb->get_var("SHOW TABLES LIKE '$table_sponsorships'") === $table_sponsorships) {
            $tables_created[] = 'sponsorships';
        }
        if ($wpdb->get_var("SHOW TABLES LIKE '$table_email_log'") === $table_email_log) {
            $tables_created[] = 'email_log';
        }
        
        error_log("CFK Tables created successfully: " . implode(', ', $tables_created));
    }
