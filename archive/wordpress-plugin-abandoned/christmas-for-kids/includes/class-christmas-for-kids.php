...
class Christmas_For_Kids {
    ...
    private function init(): void {
        // Load text domain for internationalization (early priority)
        add_action('init', [$this, 'load_textdomain'], 1);
        
        // Initialize components EARLY on init hook (before admin_menu)
        add_action('init', [$this, 'init_components'], 5);
        
        // Note: AJAX handlers are registered by individual components
        
        // Add admin menu (after components are loaded)
    ...
    public function init_components(): void {
        try {
            ...
            // Initialize Child Manager FIRST (it registers the post type)
            ...
            // Initialize Public Frontend (always for shortcodes)
    ...
    public function add_admin_menu(): void {
        ...
        // Add Dashboard as first submenu (replaces auto-generated one)
        add_submenu_page(
            'christmas-for-kids',
            __('Dashboard', CFK_TEXT_DOMAIN),
            __('Dashboard', CFK_TEXT_DOMAIN),
            'manage_options',
            'christmas-for-kids',
            [$this, 'admin_page_callback']
        ...
        // Add Children management submenus with custom pages
        add_submenu_page(
            'christmas-for-kids',
            ...
            'manage_options',
            'cfk-all-children',
            [$this, 'all_children_page_callback']
        );
        ...
        add_submenu_page(
            ...
            'manage_options',
            'cfk-add-child',
            [$this, 'add_child_page_callback']
        );
        
        add_submenu_page(
            'christmas-for-kids',
            __('Edit Child', CFK_TEXT_DOMAIN),
            __('Edit Child', CFK_TEXT_DOMAIN),
            'manage_options',
            'cfk-edit-child',
            [$this, 'edit_child_page_callback']
        );
    ...
    public function admin_page_callback(): void {
        $child_count = wp_count_posts('cfk_child')->publish;
        ...
        echo '<p>';
        echo '<a href="' . admin_url('admin.php?page=cfk-add-child') . '" class="button button-primary">' . esc_html__('Add New Child', CFK_TEXT_DOMAIN) . '</a> ';
        echo '<a href="' . admin_url('admin.php?page=cfk-import-csv') . '" class="button">' . esc_html__('Import Children from CSV', CFK_TEXT_DOMAIN) . '</a> ';
        echo '<a href="' . admin_url('admin.php?page=cfk-all-children') . '" class="button">' . esc_html__('View All Children', CFK_TEXT_DOMAIN) . '</a>';
        echo '</p>';
        echo '</div>';
    ...
    /**
     * All Children page callback
     * 
     * @since 1.0.0
     * @return void
     */
    public function all_children_page_callback(): void {
        ...
        require_once CFK_PLUGIN_PATH . 'admin/class-cfk-children-list-table.php';
        
        // Create an instance of our custom list table
        $children_table = new CFK_Children_List_Table();
        $children_table->prepare_items();
        
        echo '<div class="wrap">';
        echo '<h1 class="wp-heading-inline">' . esc_html__('All Children', CFK_TEXT_DOMAIN) . '</h1>';
        echo '<a href="' . esc_url(admin_url('admin.php?page=cfk-add-child')) . '" class="page-title-action">' . 
             esc_html__('Add New', CFK_TEXT_DOMAIN) . '</a>';
        ...
        if (isset($_GET['message'])) {
            ...
            $messages = [
                1 => __('Child added successfully.', CFK_TEXT_DOMAIN),
                2 => __('Child updated successfully.', CFK_TEXT_DOMAIN),
                3 => __('Child deleted successfully.', CFK_TEXT_DOMAIN),
            ];
            
            if (isset($messages[$message_id])) {
                echo '<div class="notice notice-success is-dismissible"><p>' . 
                     esc_html($messages[$message_id]) . '</p></div>';
            }
        }
        
        // Display the table
        echo '<form method="post">';
        $children_table->display();
        echo '</form>';
    ...
    /**
     * Add New Child page callback
     * 
     * @since 1.0.0
     * @return void
     */
    public function add_child_page_callback(): void {
        // Handle form submission
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cfk_add_child_nonce'])) {
            if (!wp_verify_nonce($_POST['cfk_add_child_nonce'], 'cfk_add_child')) {
                wp_die(__('Security check failed.', CFK_TEXT_DOMAIN));
            }
            
            // Process the form and create the child
            $child_id = $this->create_child_from_form($_POST);
            
            if ($child_id) {
                wp_redirect(admin_url('admin.php?page=cfk-all-children&message=1'));
                exit;
            } else {
                echo '<div class="notice notice-error"><p>' . 
                     esc_html__('Error creating child. Please try again.', CFK_TEXT_DOMAIN) . 
                     '</p></div>';
            }
        }
        
        $this->render_child_form();
    }
    
    /**
     * Edit Child page callback
     * 
     * @since 1.0.0
     * @return void
     */
    public function edit_child_page_callback(): void {
        $child_id = isset($_GET['child_id']) ? intval($_GET['child_id']) : 0;
        
        if (!$child_id || get_post_type($child_id) !== 'cfk_child') {
            wp_die(__('Invalid child ID.', CFK_TEXT_DOMAIN));
        }
        
        // Handle form submission
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cfk_edit_child_nonce'])) {
            if (!wp_verify_nonce($_POST['cfk_edit_child_nonce'], 'cfk_edit_child_' . $child_id)) {
                wp_die(__('Security check failed.', CFK_TEXT_DOMAIN));
            }
            
            // Process the form and update the child
            $updated = $this->update_child_from_form($child_id, $_POST);
            
            if ($updated) {
                wp_redirect(admin_url('admin.php?page=cfk-all-children&message=2'));
                exit;
            } else {
                echo '<div class="notice notice-error"><p>' . 
                     esc_html__('Error updating child. Please try again.', CFK_TEXT_DOMAIN) . 
                     '</p></div>';
            }
        }
        
        $this->render_child_form($child_id);
    }
    ...
    public function csv_import_page_callback(): void {
        // Ensure components are loaded if not already
    ...
    public function admin_enqueue_scripts(string $hook_suffix): void {
        ...
        if (strpos($hook_suffix, 'christmas-for-kids') === false && 
            ...
            (get_current_screen()->post_type ?? '') !== 'cfk_child') {
            return;
        }
    ...
    public static function activate(): void {
        ...
        try {
            // Initialize components to register post types
            $instance->init_components();
            
            // Ensure post types are registered immediately during activation
            if (isset($instance->components['child_manager'])) {
                $instance->components['child_manager']->register_post_type();
            }
    ...
    /**
     * Create a new child from form data
     * 
     * @since 1.0.0
     * @param array<string, mixed> $form_data The form data
     * @return int|false The child ID on success, false on failure
     */
    private function create_child_from_form(array $form_data): int|false {
        // Create the post
        $child_data = [
            'post_title' => sanitize_text_field($form_data['child_name'] ?? ''),
            'post_type' => 'cfk_child',
            'post_status' => 'publish',
            'meta_input' => []
        ];
        ...
        // Add meta fields
        $meta_fields = [
            'cfk_child_age' => 'child_age',
            'cfk_child_gender' => 'child_gender',
            'cfk_child_grade' => 'child_grade',
            'cfk_child_school' => 'child_school',
            'cfk_child_family_number' => 'child_family_number',
            'cfk_child_family_name' => 'child_family_name',
            'cfk_child_wishes' => 'child_wishes',
            'cfk_child_notes' => 'child_notes'
        ];
        
        foreach ($meta_fields as $meta_key => $form_key) {
            if (isset($form_data[$form_key])) {
                $child_data['meta_input'][$meta_key] = sanitize_text_field($form_data[$form_key]);
            }
        }
        
        return wp_insert_post($child_data);
    }
    
    /**
     * Update a child from form data
     * 
     * @since 1.0.0
     * @param int $child_id The child ID to update
     * @param array<string, mixed> $form_data The form data
     * @return bool True on success, false on failure
     */
    private function update_child_from_form(int $child_id, array $form_data): bool {
        // Update the post
        $child_data = [
            'ID' => $child_id,
            'post_title' => sanitize_text_field($form_data['child_name'] ?? ''),
        ...
        $updated = wp_update_post($child_data);
        ...
        foreach ($meta_fields as $meta_key => $form_key) {
            if (isset($form_data[$form_key])) {
                update_post_meta($child_id, $meta_key, sanitize_text_field($form_data[$form_key]));
            }
        }
    ...
    /**
     * Render the child form (for both add and edit)
     * 
     * @since 1.0.0
     * @param int|null $child_id The child ID for editing, null for adding
     * @return void
     */
    private function render_child_form(?int $child_id = null): void {
        $is_edit = $child_id !== null;
        $title = $is_edit ? __('Edit Child', CFK_TEXT_DOMAIN) : __('Add New Child', CFK_TEXT_DOMAIN);
        $child_data = [];
        ...
        if ($is_edit) {
            ...
            $child_data = [
                ...
                'wishes' => get_post_meta($child_id, 'cfk_child_wishes', true),
                'notes' => get_post_meta($child_id, 'cfk_child_notes', true)
            ];
        }
        ...
        if ($is_edit) {
            wp_nonce_field('cfk_edit_child_' . $child_id, 'cfk_edit_child_nonce');
        } else {
            wp_nonce_field('cfk_add_child', 'cfk_add_child_nonce');
        }
        ...
        // Child Name
        echo '<tr>';
        echo '<th scope="row"><label for="child_name">' . esc_html__('Child Name', CFK_TEXT_DOMAIN) . '</label></th>';
        echo '<td><input type="text" id="child_name" name="child_name" value="' . 
             esc_attr($child_data['name'] ?? '') . '" class="regular-text" required /></td>';
        echo '</tr>';
        
        // Age
        echo '<tr>';
        echo '<th scope="row"><label for="child_age">' . esc_html__('Age', CFK_TEXT_DOMAIN) . '</label></th>';
        echo '<td><input type="number" id="child_age" name="child_age" value="' . 
             esc_attr($child_data['age'] ?? '') . '" min="1" max="18" class="small-text" /></td>';
        echo '</tr>';
        
        // Gender
        echo '<tr>';
        echo '<th scope="row"><label for="child_gender">' . esc_html__('Gender', CFK_TEXT_DOMAIN) . '</label></th>';
        echo '<td>';
        echo '<select id="child_gender" name="child_gender">';
        echo '<option value="">' . esc_html__('Select Gender', CFK_TEXT_DOMAIN) . '</option>';
        $current_gender = $child_data['gender'] ?? '';
        foreach (['Male', 'Female', 'Other'] as $gender) {
            $selected = selected($current_gender, $gender, false);
        ...
        // Grade
        echo '<tr>';
        echo '<th scope="row"><label for="child_grade">' . esc_html__('Grade', CFK_TEXT_DOMAIN) . '</label></th>';
        echo '<td><input type="text" id="child_grade" name="child_grade" value="' . 
             esc_attr($child_data['grade'] ?? '') . '" class="regular-text" /></td>';
        echo '</tr>';
        
        // School
        echo '<tr>';
        echo '<th scope="row"><label for="child_school">' . esc_html__('School', CFK_TEXT_DOMAIN) . '</label></th>';
        echo '<td><input type="text" id="child_school" name="child_school" value="' . 
             esc_attr($child_data['school'] ?? '') . '" class="regular-text" /></td>';
        echo '</tr>';
        
        // Family Number
        echo '<tr>';
        echo '<th scope="row"><label for="child_family_number">' . esc_html__('Family Number', CFK_TEXT_DOMAIN) . '</label></th>';
        echo '<td><input type="text" id="child_family_number" name="child_family_number" value="' . 
             esc_attr($child_data['family_number'] ?? '') . '" class="regular-text" /></td>';
        echo '</tr>';
        
        // Family Name
        echo '<tr>';
        echo '<th scope="row"><label for="child_family_name">' . esc_html__('Family Name', CFK_TEXT_DOMAIN) . '</label></th>';
        echo '<td><input type="text" id="child_family_name" name="child_family_name" value="' . 
             esc_attr($child_data['family_name'] ?? '') . '" class="regular-text" /></td>';
        echo '</tr>';
        
        // Wishes
        echo '<tr>';
        echo '<th scope="row"><label for="child_wishes">' . esc_html__('Wishes', CFK_TEXT_DOMAIN) . '</label></th>';
        echo '<td><textarea id="child_wishes" name="child_wishes" rows="4" class="large-text">' . 
             esc_textarea($child_data['wishes'] ?? '') . '</textarea></td>';
        echo '</tr>';
        
        // Notes
        echo '<tr>';
        echo '<th scope="row"><label for="child_notes">' . esc_html__('Notes', CFK_TEXT_DOMAIN) . '</label></th>';
        echo '<td><textarea id="child_notes" name="child_notes" rows="4" class="large-text">' . 
             esc_textarea($child_data['notes'] ?? '') . '</textarea></td>';
        echo '</tr>';
        ...
        echo '<input type="submit" class="button-primary" value="' . 
             esc_attr($is_edit ? __('Update Child', CFK_TEXT_DOMAIN) : __('Add Child', CFK_TEXT_DOMAIN)) . '" />';
        echo ' <a href="' . esc_url(admin_url('admin.php?page=cfk-all-children')) . '" class="button">' . 
             esc_html__('Cancel', CFK_TEXT_DOMAIN) . '</a>';
        echo '</p>';
